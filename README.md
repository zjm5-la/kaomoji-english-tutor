...
- **按需备课**：宠物从最近对话中识别主题。模型判定信息不足后，如果会话没有变化就不会重复请求；条件满足后生成 1 个单词、1 个词组和 1 个语法填空。新卡只在首次真正展示时消耗每日额度；Skip 补卡严格一换一、额度外，并让位于已到期复习
...
- **语法填空**：每道题是一句英文恰挖一个空（空后括号给原形提示），专考时态、语态、主谓一致、介词等明确语法点，答案唯一；本地精确评判、SDK LLM 兄底。更早生成的渐进默写句子卡不再新增，存量卡片照常复习（L1 填空 → L2 分句 → L3 整句）
