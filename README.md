...
- **按需备课**：条件满足后生成一天的完整批次：10 个单词或词组（以单词为主，词组最多 3 个）加 1 个语法填空。学习者备考雅思：单词/词组至少 3 个来自当前会话的真实表达，其余从雅思入门/基础段（4.0-5.5 分，A2-B1）高频常用核心词选取（贴合画像词汇档，初学阶段禁超纲学术难词，critic 把关）；会话缺乏英语内容时也用雅思词汇出满一批。新卡只在首次真正展示时消耗每日额度；Skip 补卡严格一换一、额度外，并让位于已到期复习，补卡同样可用雅思词汇兜底
