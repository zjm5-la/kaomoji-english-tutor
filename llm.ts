...
export async function critiqueLesson(
	...
	const prompt = [
		...
		"审查标准：",
		"- 英语单词/词组/句子必须正确、自然",
		"- 词汇难度须符合画像「词汇层次」：词汇档低于 B2 时出现生僻学术词、低频难词记 blocker；雅思来源的词必须是初学者常用的基础核心词",
...
export async function generateReplacement(
	...
	const prompt = [
		...
		"优先从下面会话中提取真实表达；会话没有合适内容时，从雅思入门/基础段（约 4.0-5.5 分，A2-B1）的高频常用核心词汇中选择同类型卡片，学习者是初学者，禁止生僻学术词、低频难词，不要因为会话缺乏英语内容而拒绝。",
